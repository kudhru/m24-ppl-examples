(*
    Easy-Level OCaml Question: Representing and Manipulating a File System
...
let rec total_size (fs: fs) : int =
  match fs with 
  | File (_, size) -> size
  | Dir (_, contents) -> List.fold_left (fun acc subdir -> acc + total_size subdir) 0 contents
...
let rec to_string (fs: fs) (indent: int) : string =
  match fs with
  | File (name, size) -> (List.fold_left (fun acc x -> acc ^ x) "" (List.init indent (fun _ -> " "))) ^ name ^ " (" ^ string_of_int size ^ ")\n"
  | Dir (name, contents) -> (List.fold_left (fun acc x -> acc ^ x) "" (List.init indent (fun _ -> " "))) ^ name ^ "/\n" ^ String.concat "" (List.map (fun x -> to_string x (indent + 1)) contents)
