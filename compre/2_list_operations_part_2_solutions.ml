...
*)
let rec split_list_helper (lst: 'a list) (acc: 'a list * 'a list) : 'a list * 'a list =
  match lst with
  | [] -> acc
  | head::tail -> 
    split_list_helper tail (if (List.length (fst acc)) = (List.length (snd acc)) then (fst acc @ [head], snd acc) else (fst acc, snd acc @ [head]))

let rec split_list (lst: 'a list) : 'a list * 'a list =
  split_list_helper lst ([], [])
...
*)
let rec split_list_from_left (lst: 'a list) (n: int) (acc: 'a list * 'a list) : ('a list * 'a list) =
  match lst, n with
  | [], _ -> acc
  | head::tail, 0 -> acc
  | head::tail, n -> split_list_from_left tail (n - 1) (tail, snd acc @ [head])

let rec rotate_list (lst: 'a list) (n: int) : 'a list =
  let (left, right) = split_list_from_left lst n ([], []) in
  left @ right
...
*)
let rec flatten_nested_list_helper (nested_lst: 'a list list) (acc: 'a list) : 'a list =
  match nested_lst with
  | [] -> acc
  | head::tail -> flatten_nested_list_helper tail (acc @ head)

let rec flatten_nested_list (nested_lst: 'a list list) : 'a list =
  flatten_nested_list_helper nested_lst []
...
let rec pairwise_sum (lst1: int list) (lst2: int list) : int list =
  ...
  | [], _ -> []
  | _, [] -> []
  | head1::tail1, head2::tail2 -> (head1 + head2)::(pairwise_sum tail1 tail2)
