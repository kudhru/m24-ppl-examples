...
let rec collect_descendants tree = 
  match tree with
  | Leaf name -> [name]
  | Node (name, children) -> name::(List.concat (List.map collect_descendants children))
...
let rec find_descendants (tree: family_tree) (person: string) : string list =
  ...
  | Node (name, children) -> 
    if name = person then 
      List.concat (List.map collect_descendants children) 
    else 
      List.concat (List.map (fun x -> find_descendants x person) children)
(*
    Function: tree_depth
    Description:
    - Calculates the depth of the family tree.
...
let rec tree_depth (tree: family_tree) : int =
  match tree with
  | Leaf _ -> 1
  | Node (_, children) -> 1 + List.fold_left (fun acc child -> max acc (tree_depth child)) 0 children
...
let rec to_string (tree: family_tree) (indent: int) : string =
  match tree with
  | Leaf name -> (List.fold_left (fun acc x -> acc ^ x) "" (List.init indent (fun _ -> " "))) ^ name ^ "\n"
  | Node (name, children) -> (List.fold_left (fun acc x -> acc ^ x) "" (List.init indent (fun _ -> " "))) ^ name ^ "\n" ^ String.concat "" (List.map (fun x -> to_string x (indent + 1)) children)
...
let main () =
  ...
  Printf.printf "Descendants of Carol: %s\n"
    (String.concat ", " (find_descendants tree "Carol"));
  Printf.printf "Descendants of David: %s\n"
    (String.concat ", " (find_descendants tree "David"));
